    ...
    <form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>">
        <div class="section_wrapper">
            <h3 class="title_black"><?php echo $order->client_name . ' &ndash; ' . $this->lang->line('order_number') . ' ' . $order->order_number; ?>
                ...
                <?php } ?>
                <!--<input type="submit" name="download_order_items" style="float: right; margin-top: 10px; margin-right: 10px;" value="CSV »" />-->    
                <button class="btn btn-primary" id="export_csv_selected" style="float: right; margin-top: 14px; margin-right: 10px;">CSV »</button>
                ...
                <form method="post" action="<?= site_url() ?>/orders/edit/order_id/<?= $order->order_id ?>" style="display: inline;">
                    <input type="submit" id="order_send_email" name="btn_send_email" style="float: right; margin-top: 10px; margin-right: 10px;" value="Send Email">
                </form>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $('#order_send_email').on('click', function () {
            var contact_id = '<?= $order->contact_id ?>';
            var sel_contact = $('#contact_id').val();
            if (contact_id > 0) {
                if (contact_id == sel_contact) {
                } else {
                    alert('Please select contact and click "Save Options" to save before sending..');
                    return false;
                }
            } else {
                alert('Please select contact and click "Save Options" to save before sending..');
