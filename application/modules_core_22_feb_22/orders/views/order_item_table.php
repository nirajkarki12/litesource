...
?>
    ...
    var edit_order_status = '<?=$order->order_status?>';
...
//        {id: "item_action", name: "", field: "item_action", cssClass: "cell-center-align", formatter: addRowIcon}
    ...
    var order_status = <?php echo $order->order_status_id; ?>;

    var options = {
        autoEdit: false,
        editable: (order_status == 1 ? true : false),
    ...
    function asyncRenderItemLink(cellNode, row, dataContext, colDef) {
        ...
        if (dataContext[f] == null) return;
        var a = '<a style="padding: 4px;padding-left: 8px;background: #e0e0e0;\n\
                            padding-right: 8px;"\n\
                            href="javascript:void(0)"\n\
                            onclick="chooseQty(\''
                            + colDef.linkUrl + '/<?= uri_assoc('order_id')?>/' + dataContext[fl] + '\',\'' 
                            + dataContext.item_qty + '\',\'' 
                            + dataContext.partial_qty + '\',\'' 
                            + dataContext.item_name + '\''
                            + ')">' + dataContext[f] + '</a>';
    ...
    function chooseQty(url, qty, partial_qty, name){
        
        if( edit_order_status == 'Open' ){
            event.preventDefault();
            alert('This order has status open. Please change it to Emailed to perform the stock in.');
            return false;
        }
        
        if(partial_qty > 0 && (partial_qty - qty >= 0)) {
        ...
        var remainingQty = (qty - partial_qty).toFixed(2);
        remainingQty=parseFloat(remainingQty);
        
        qty = Math.round(qty);
        partial_qty = Math.round(partial_qty);
        
        //console.log(partial_qty);
        ...
        var bcr_popup_content = '<form action="'+url+'" method="post"> \n\
                                <div class="lite_model-header"><span class="lite_model_close" onclick="lite_close_popup()">&times;</span>\n\
                                Enter Quantity you want to stock in for <strong><em>' + name +'</em></strong>\n\
                                </div>\n\
                                <div class="lite_model-body">\n\
                                    <div style="width:100%; display: inline-flex;">\n\
                                        <div style="width:50%;">\n\
                                            <h6>Total Order Qty: ' + qty +'</h6><br>\n\
                                            <h6>Stocked In Quantity: ' + partial_qty +'</h6><br>\n\
                                            <h6>Available Qty to stock in: ' + remainingQty +'</h6><br>\n\
                                        </div>\n\
                                    </div>\n\
                                    <div style="text-align: center;width="100%;">\n\
                                        <h6>Enter Qty to stock in:</h6>\n\
                                        <input type="number" step=0.01 name="partial_qty" min="0.01" max="' + remainingQty +'" value=' + remainingQty +' style="width: 75px;height: 18px;margin-top: 10px;">\n\
...
//            grid.getActiveCell()
    ...
    $(document).on('click', '#export_csv_selected', function (e) {
        ...
        if (items_to_csv_export.length > 0) {
            ...
            var data_to_export = {items_to_csv_export:items_to_csv_export}
            $.ajax({
                url: "<?= site_url('orders/export_selected_items'); ?>",
                type: 'POST',
                data: data_to_export,
                success: function (data) {
                    if( data ){
                        var file_name = "Order_selected_"+order_num+".csv";
