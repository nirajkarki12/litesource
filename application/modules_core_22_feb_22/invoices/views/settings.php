...
<dl>
    ...
    <dd>
		<select name="default_tax_rate_option">
			<option value="1" <?php if ($this->mdl_mcb_data->setting('default_tax_rate_option') == 1) { ?>selected="selected"<?php } ?>><?php echo $this->lang->line('item_tax_option_1'); ?></option>
			<option value="2" <?php if ($this->mdl_mcb_data->setting('default_tax_rate_option') == 2) { ?>selected="selected"<?php } ?>><?php echo $this->lang->line('item_tax_option_2'); ?></option>
		</select>
    </dd>
<dl>
...
<dl>
    <dt><?php echo $this->lang->line('default_pdf_info_user'); ?></dt>
    <dd>
	<select name="default_pdf_info_user_id">
	    <?php foreach ($users as $user) { ?>
	    <option value="<?php echo $user->user_id; ?>" <?php if($this->mdl_mcb_data->setting('default_pdf_info_user_id') == $user->user_id) { ?>selected="selected"<?php } ?>><?php echo $user->username; ?></option>
	    <?php } ?>
	</select>
    </dd>
</dl>
...
<dl>
    <dt><?php echo $this->lang->line('default_order_delivery_address'); ?></dt>
    <dd>
	<select name="default_order_address_id">
	    <?php foreach ($addresses as $address) { ?>
	    <option value="<?php echo $address->address_id; ?>" <?php if($this->mdl_mcb_data->setting('default_order_address_id') == $address->address_id) { ?>selected="selected"<?php } ?>><?php echo $address->address_formatted; ?></option>
	    <?php } ?>
	</select>
    </dd>
</dl>
...
<dl>
	<dt><?php echo $this->lang->line('disable_invoice_payments'); ?></dt>
	<dd><input type="checkbox" name="disable_invoice_payments" value="1" <?php if ($this->mdl_mcb_data->setting('disable_invoice_payments')) { ?>checked="checked"<?php } ?>/></dd>
</dl>
</div>