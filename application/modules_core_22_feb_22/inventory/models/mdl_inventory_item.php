...
//                $result[$i['order_id']] = $i;
    ...
    function get_pending_qty($inv_id, $inv_pros = array()) {
        if(!$pendingQtyDetails =  $this->getPendingQuantityData($inv_id)) {
            $pendingQtyDetails =  $this->getPendingQuantity($inv_id);
        }
    ...
    function getPendingQuantity($inv_id) {
                ...
                if ($qry_res) {
                    ...
                    if ($tot_invoice != NULL) {
                        ...
                        foreach ($tmp_invoices as $invoice) {
                            $invoice['pid'] = $inv_id;
                            $invoice['inventory_qty'] = $pi['inventory_qty'];
        ...
        if ($associated_invoices != NULL) {
            ...
            foreach ($tmp_invoices as $invoice) {
                ...
                if(isset($invoice['inventory_qty']) && (int)$invoice['inventory_qty'] > 0 && $invoice['item_qty'] != ''){
                    $invoice['item_qty'] = $invoice['item_qty'] * $invoice['inventory_qty'];
                }
            ...
            foreach ($tot_invoice as $key => $invc) {
                ...
                if ($i_ststus_id == '3' || $i_ststus_id == '4') {//escaping overdue status also
...
//        die;
    ...
    function get_pending_qty_by_inv_id($inv_id, $inv_pros) {
        $result = array();
        $associated_invoices = array();
            ...
            foreach ($inv_pros as $pi) {
                ...
                if ($qry_res) {
                    ...
                    if ($tot_invoice != NULL) {
                        ...
                        foreach ($tmp_invoices as $invoice) {
                            $invoice['pid'] = $inv_id;
                            $invoice['inventory_qty'] = isset($pi['inventory_qty']) ? $pi['inventory_qty'] : null;
                            $associated_invoices[] = $invoice;
                        }
                    }
                }
        ...
        if ($associated_invoices != NULL) {
            ...
            foreach ($tot_invoice as $key => $invc) {
                $invoice_id = $invc['invoice_id'];
                $i_ststus_id = $this->mdl_invoices->getInvoiceStatusId($invoice_id);
                if ($i_ststus_id == '3' || $i_ststus_id == '4') {//esaping overdue status
                    unset($tot_invoice[$key]);
                } else {
                    $inv_id = $invc['pid'];
                    $result[$key] = $invc;
...
//        die;
        ...
        $qty = 0;
        if(sizeof($final_result) != NULL) {
            $i = 0;
            $sum = 0;
            foreach ($final_result as $inv) {
                if( $inv['incident'] =='Invoice item' ){
                    $sum += $inv['item_qty'];
                    $i++;
                }else{
    ...
    function get_pending_qty_group_by_inv_id($inv_id) {
        $qty = 0;
        ////// getting all invoice items
        $query_i = "SELECT 
                    ii.invoice_id, 
                    ...
                    left join mcb_invoices as i on i.invoice_id = ii.invoice_id 
                    WHERE ii.`product_id`='{$inv_id}' AND i.invoice_is_quote ='0'";
        $_invoices = $this->db->query($query_i);
        ...
        if ($_invoices != NULL) {
            ...
            $invoices = $_invoices->result_array();
            $invoices = $this->deduct_docket_qty($invoices);
            ...
            foreach ($invoices as $invoice) {
                ...
                $i_ststus_id = $this->mdl_invoices->getInvoiceStatusId($invoice['invoice_id']);
                if ($i_ststus_id == 3 || $i_ststus_id == '4') continue;

                if ($invoice['item_qty'] != '') {
                    $tmp_qty = number_format($invoice['item_qty'], 2);
                }
                if ($invoice['item_length'] != '') {
                    $tmp_length = number_format($invoice['item_length'], 2);
                }
                /// if item length
                if (($tmp_length != '0') && ($tmp_length != '')) {
                    $tmp_qty = $tmp_qty * $tmp_length;
                }
                $qty += $tmp_qty;
    ...
    function getPendingQuantityData($inv_id) {
        $qty = 0;
        ////// getting all invoice items
        $inv_type_q = $this->db->query("SELECT inventory_type FROM `mcb_inventory_item` WHERE `inventory_id`='{$inv_id}'");
        $inv_type = $inv_type_q->row();
        if(!$inv_type->inventory_type) return false;
        ...
        $query_i = "SELECT 
                    ...
                    mii.inventory_type
                    FROM `mcb_invoice_items` AS ii 
                    JOIN `mcb_inventory_item` mii ON mii.inventory_id= ii.product_id
                    left join mcb_invoices as i on i.invoice_id = ii.invoice_id 
                    WHERE ii.`product_id`='{$inv_id}' AND i.invoice_is_quote ='0'";
        $_invoices = $this->db->query($query_i);
        ...
        if ($_invoices != NULL) {
            ...
            foreach ($invoices as $key => $invoice) 
            {
                ...
                if ($i_ststus_id == 3 || $i_ststus_id == '4') {
                    unset($invoices[$key]);
                    continue;
                }
    ...
    function deduct_docket_qty($invoices)
    {
        if($invoices)
        {
            $invoice_ids = array_column($invoices, 'invoice_id');
            $invoice_ids_cond = "('" .implode("','", $invoice_ids) ."')";

            $query_i = "SELECT 
                        ddi.docket_item_qty, 
                        ddi.docket_item_length, 
                        ii.product_id,
                        dd.invoice_sent,
                        dd.paid_status,
                        dd.docket_delivery_status,
                        dd.invoice_id
                        FROM mcb_delivery_dockets AS dd
                        JOIN mcb_delivery_docket_items AS ddi ON ddi.docket_id = dd.docket_id
                        JOIN mcb_invoice_items AS ii ON ii.invoice_item_id = ddi.invoice_item_id
                        WHERE dd.invoice_id IN $invoice_ids_cond";
            $_dockets = $this->db->query($query_i);
            if ($_dockets != NULL) {

                $dockets = $_dockets->result_array();
                $docket_arr = [];
                foreach($dockets as $docket)
                {
                    $docket_arr[$docket['invoice_id']][$docket['product_id']][] = $docket;
                }

            }
            // echo "<pre>";print_r($docket_arr);print_r($invoices);

            if($docket_arr) {
                foreach($invoices as $invoice_key => $invoice) {

                    if( $docket_arr[$invoice['invoice_id']] ){
                        foreach( $docket_arr[$invoice['invoice_id']] as $product_id => $dockets) {
                            foreach($dockets as $docket)
                            {

                                if( $docket['docket_delivery_status'] == '1' ) { //deducting delivered dockets
                                    if( $invoice['product_id'] == $product_id ) {
                                        if ( is_numeric($docket['docket_item_length']) && $docket['docket_item_length'] != '0' ) {
                                            $docket['docket_item_qty'] = $docket['docket_item_qty'] * $docket['docket_item_length'];
                                        }
                                        if( $docket['docket_item_qty'] > '0' ) {
                                            $invoices[$invoice_key]['item_qty'] -= $docket['docket_item_qty'];
                                        } else {
                                            $invoices[$invoice_key]['item_qty'] += $docket['docket_item_qty'];
                                        }
                                    }
    ...
    function get_suplier_inventory($limit = NULL, $offset = '0', $filters = []){
        ...
        $select = "
                ...
                i.`use_length`, 
                i.`description`, 
                CASE WHEN i.`supplier_price` IS NULL THEN '$0.00' ELSE CONCAT('$', FORMAT(i.supplier_price, 2)) END AS s_p,
                i.`supplier_code` AS s_c,
                CONCAT('$', FORMAT(i.`base_price`, 2)) base_price, 
                i.`supplier_description`, 
                i.`location`, 
                i.`o_qty`, 
                '0' AS p_q,
                CASE WHEN i.inventory_type = '1' THEN 'Product Group' ELSE 'Part' END AS i_t,
                (
                    SELECT SUM( (oit.item_qty - oit.partial_qty) * IF(oit.item_length > 0,oit.item_length,1)) 
                    FROM `mcb_order_inventory_items` AS oit 
                    LEFT JOIN mcb_orders AS ord ON (oit.order_id = ord.order_id) 
                    WHERE oit.inventory_id = i.inventory_id AND 
                    ord.order_status_id = '2' AND oit.stock_status!='1'
                ) as o_q,
                CASE WHEN i.qty IS NOT NULL THEN i.qty ELSE '0.00' END AS qty
                ";

        $sql = "SELECT SQL_CALC_FOUND_ROWS $select
                FROM `mcb_inventory_item` AS i 
                LEFT JOIN `mcb_clients` AS `client` ON client.client_id = i.`supplier_id`
                WHERE i.is_arichved != '1'";

        $where_cond = null;
        $filter_added = false;

        if($filters && is_array($filters) && count($filters) > 0){
            if(array_key_exists('product_type', $filters) && $filters['product_type'] != 'false'){
                $where_cond .= sprintf(" AND (CASE WHEN i.inventory_type = '1' THEN 'product group' ELSE 'part' END) LIKE '%%%s%%'", strtolower($filters['product_type']));
            }
            if(array_key_exists('base_price', $filters) && $filters['base_price'] != 'false'){
                $where_cond .= sprintf(" AND CONCAT('$', FORMAT(i.`base_price`, 2)) LIKE '%%%s%%'", $filters['base_price']);
            }
            if(array_key_exists('description', $filters) && $filters['description'] != 'false'){
                $where_cond .= sprintf(" AND LOWER(i.description) LIKE '%%%s%%'", strtolower($filters['description']));
            }
            if(array_key_exists('cat_name', $filters) && $filters['cat_name'] != 'false'){
                $where_cond .= sprintf(" AND LOWER(i.name) LIKE '%%%s%%'", strtolower($filters['cat_name']));
            }
            if(array_key_exists('qty', $filters) && $filters['qty'] != 'false'){
                $where_cond .= sprintf(" AND i.qty = '%s'", $filters['qty']);
            }
            if(array_key_exists('sku', $filters) && $filters['sku'] != 'false'){
                $where_cond .= sprintf(" AND LOWER(i.sku) LIKE '%%%s%%'", strtolower($filters['sku']));
            }
            if(array_key_exists('supplier_code', $filters) && $filters['supplier_code'] != 'false'){
                $where_cond .= sprintf(" AND LOWER(i.supplier_code) LIKE '%%%s%%'", strtolower($filters['supplier_code']));
            }
            if(array_key_exists('supplier_name', $filters) && $filters['supplier_name'] != 'false'){
                if(preg_match("/(new)/i", strtolower($filters['supplier_name']))){
                    $where_cond .= " AND client.client_id IS NULL";
                } else{
                    $where_cond .= sprintf(" AND LOWER(client.client_name) LIKE '%%%s%%'", strtolower($filters['supplier_name']));
                }
            }
            if(array_key_exists('supplier_price', $filters) && $filters['supplier_price'] != 'false'){
                $where_cond .= sprintf(" AND (CASE WHEN i.`supplier_price` IS NULL THEN '$0.00' ELSE CONCAT('$', FORMAT(i.supplier_price, 2)) END) LIKE '%%%s%%'", $filters['supplier_price']);
            }

            if(array_key_exists('open_order_qty', $filters) && $filters['open_order_qty'] != 'false'){
                $where_cond .= sprintf(" AND (
                    SELECT SUM( (oit.item_qty - oit.partial_qty) * IF(oit.item_length > 0,oit.item_length,1)) 
                    FROM `mcb_order_inventory_items` AS oit 
                    LEFT JOIN mcb_orders AS ord ON (oit.order_id = ord.order_id) 
                    WHERE oit.inventory_id = i.inventory_id AND 
                    ord.order_status_id = '2' AND oit.stock_status!='1'
                ) = '%s'", $filters['open_order_qty']);
            }
        ...
        $sql .= " ORDER BY client.client_name ASC LIMIT $offset, $limit";
        ...
        return ['inventory' => $inventory, 'total_data' => count($inventory), 'filters' => $filter_added];

    }
    
    function get_pending_qty__old($inv_id, $inv_pros = array()) {
...
//                . "WHERE product_name NOT IN (SELECT name FROM `mcb_inventory_item`)";
    ...
    function maintain_group_product_quantity($invoice_id, $is_quote = false) {
        $sql = "SELECT * 
            FROM mcb_orders AS o
            LEFT JOIN mcb_order_inventory_items AS oii ON o.order_id = oii.order_id
            LEFT JOIN mcb_invoices AS inv ON inv.invoice_quote_id = o.invoice_id
            WHERE 1=1
            ";
        if($is_quote) {
            $sql .= " AND inv.invoice_id = '$invoice_id'";
        } else{
            $sql .= " AND o.invoice_id = '$invoice_id'";
        }
        $order_q = $this->db->query($sql);

        $orders = $order_q->result_array();
        $tmp_array = array();
        foreach($orders as $order) {
            $tmp_array[$order['product_id']][] = $order;
            $invoice_id = $order['invoice_id'];
        }

        $is_order_completed = false;
        $this->load->model('invoices/mdl_invoices');
        $this->load->model('delivery_dockets/mdl_invoices');
        $inv_status = $this->mdl_invoices->getInvoiceStatusId($invoice_id);

        $delivery_dockets = $this->common_model->get_all_as_object('mcb_delivery_dockets', array('invoice_id' => $invoice_id));

        $all_docket_sent = false;
        if($delivery_dockets) {
            $all_docket_sent = true;

            foreach($delivery_dockets as $docket) {
                if( (isset($docket->docket_delivery_status) && $docket->docket_delivery_status == '0') ) {
                    $all_docket_sent = false;
                    break;
                }
            }
        ...
        foreach($tmp_array as $product => $orders) {
            if(count($orders) > 0 ) {
                ...
                $invoice_q = $this->db->query("
                            SELECT * 
                            FROM mcb_invoices AS i
                            LEFT JOIN mcb_invoice_items AS ii ON i.invoice_id = ii.invoice_id
                            WHERE i.invoice_id = '$invoice_id' AND ii.product_id = '$product'"
                        );
                $invoice = $invoice_q->row();

                if($is_order_completed) {

                    $inv_group_q = $this->db->query("
                            SELECT * 
                            FROM mcb_inventory_group
                            WHERE invoice_id = '$invoice_id' AND product_id = '$product'");
                    $inv_group = $inv_group_q->row();

                    if($inv_status != '3' && !$inv_group) {
                        $this->insert('mcb_inventory_group', array('product_id' => $product, 'invoice_id' => $invoice_id, 'qty' => $invoice->item_qty));
                    } elseif($inv_status == '3' && $inv_group) {
                        $this->db->delete('mcb_inventory_group', array('invoice_id' => $invoice_id, 'product_id' => $product));
                    }
