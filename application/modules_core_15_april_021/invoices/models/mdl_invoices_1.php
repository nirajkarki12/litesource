...
//                unset($value->s);
            ...
            $t_amt = (float) str_replace(",", "", $t_amt);


//                echo '<br>';
//                echo $this->invoice_paid_amount($value->id);;
//                die;

            $value->paamt = $this->invoice_paid_amount($value->id);

            if ($value->paamt > 0) {
                $value->o_amt = (float) ($t_amt) - (float) ($value->paamt);
            }
            if (isset($value->o_amt)) {
                $value->a = $value->o_amt;
                $value->a = '' . number_format($value->a, 2) . '';
            }
...
//        die;
    ...
    public function save_invoice_options($custom_fields = NULL) {
        ...
        if (($this->input->post('smart_status')) == '1') {
...
//        die;
    ...
    function add_new_invoice_item($item) {
        ...
        if (!$item->product_id > 0) {
            ...
            if ($item->item_name !== '') {
                ...
                if (isset($product)) {
                    ...
                    log_message('INFO', 'Found product id:' . $item->product_id . ' ' . $product->product_name . ' ' . $product->product_description);
                }
                if ($item->product_id < '0') {
                    $item->product_id = '0';
                }
                if ($item->item_price == '' || $item->item_price == NULL) {
        ...
        if ($this->db->insert('mcb_invoice_items', $db_array)) {
            ...
            $this->update_invoice_amounts($item->invoice_id);

            if ($invoice_item_id > 0) {
                $check_invoice = $this->get_row('mcb_invoices', array('invoice_id' => $item->invoice_id));
//               if($check_invoice->invoice_is_quote == '0'){

                $db_inc_his = array(
                    'invoice_id' => $item->invoice_id,
                    'invoice_history_date' => time(),
                    'invoice_history_data' => $item->item_name . ' Added.',
                    'user_id' => $this->session->userdata('user_id')
                );
    ...
    public function add_invoice_item($invoice_id, $product_id, $item_name, $item_description, $item_qty, $item_length, $itme_per_meter, $item_price, $item_index = 9999, $item_date = NULL) {
        ...
        if (!$product_id > 0) {
            if ($item_name !== '') {
                $this->load->model('products/mdl_products');
                // $product = $this->mdl_products->get_product_by_name($item_name);
                $product = $this->get_row('mcb_products', array('product_name' => $item_name, 'is_arichved !=' => '1'));
                if (isset($product)) {
...
//        if (( strpos($item->original_name, 'mm') !== FALSE) || ( strpos($item->original_name, 'Per-Metre') !== FALSE) ) {
//            $item->product_dynamic = 1;
//        }
...
//        }
    ...
    public function get_invoice_item($invoice_item_id) {
        ...
        $this->db->select('mcb_invoice_items.*, item_subtotal, item_tax, item_total,mcb_inventory_item.supplier_id');
        ...
        $this->db->join('mcb_inventory_item', 'mcb_inventory_item.inventory_id = mcb_invoice_items.product_id','left');
        $item = $this->db->get('mcb_invoice_items')->row();
        
        //print_r($this->db->last_query()); exit;
        
        $itm = $this->get_row('mcb_products', array('product_id' => $item->product_id));
        //var_dump($this->db->last_query()); var_dump($itm);exit();
        if ($itm != NULL) {
            $item->product_dynamic = $itm->product_dynamic;
            if ($itm->is_arichved == '1') {
                $item->is_removed = '1';
            }
        } else {
            $item->is_removed = '1';
