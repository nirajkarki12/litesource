        ...
        <table style="width: 80%;text-align: right;">
            <thead>
                <tr style="background: black;color: white">
                    ...
                    <td>Docket Amount</td>
                    <td>Owing Amount</td>
                    <td>Applied Amount(<?= currency_symbol()?>)</td>
                    <td>Remaining Amount(<?= currency_symbol()?>)</td>
                </tr>
            </thead>
            <?php if ($dockets != NULL) { ?>
                <tbody>
                    <?php foreach ($dockets as $docket) {
                        ...
                        <tr style="background: #f0f0f0;" data-id="<?=$d_id?>">
                            ...
                            <input type="hidden" id="dow<?=$d_id?>" value="<?= docket_owing_wit_out_currency_wit_coma($docket)?>">
                            <!-------applied amount------->
                            <td id="apatd<?=$d_id?>">0.00</td>
                            <input type="hidden" name="paid_amounts[]" id="apainpt<?=$d_id?>" value="0.00">
                            <!-------applied amount------->
                            <td id="remamnt<?=$d_id?>"><?= docket_owing_wit_out_currency_wit_coma($docket)?></td>
                        </tr>
                    <?php } ?>
                </tbody>
        <?php } ?>
...
<script>
    ...
    jQuery('.chkd_dkt').on("change", function (event) {
        ...
        var owng_amnt = parseFloat($('#dow'+id).val().split(',').join('')).toFixed(2);
        
        //console.log(owng_amnt);
...
//                rem_amnt = rem_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,');
                ...
                $('#apatd'+id).html(owng_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                $('#apainpt'+id).val(owng_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                ...
                if(numItems > 0){
                    ...
                    for (index = 0; index < numItems; ++index) {
                        ...
                        var r_amnt = parseFloat($('#remamnt'+i_id).text().split(',').join('')).toFixed(2);
                        ids_arr.push(r_amnt);
                    }
                    var sum = 0.00;
                    var numbers = ids_arr
                    ...
                    if( sum == 0 ){
                        if(rem_amnt != 0){
                            ...
                            $('#remamnt'+id).html(temp_re_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                            var temp_owing_amnt = (parseFloat(owng_amnt) + parseFloat(rem_amnt)).toFixed(2);   
                            $('#apatd'+id).html(temp_owing_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                            $('#apainpt'+id).val(temp_owing_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                        }       
                    }
                }
            } else {
                ...
                $('#apatd'+id).html(entred_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                $('#apainpt'+id).val(entred_amnt.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                $('#remamnt'+id).html(r_a.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
                $('#docket_paid_amount').val('0.00');
                $('#docket_paid_amount').attr('disabled',true);
            }
        }else{
            var apld_amnt = parseFloat($('#apainpt'+id).val().split(',').join('')).toFixed(2);
            var r_amnt = parseFloat($('#remamnt'+id).text().split(',').join('')).toFixed(2);
            ...
            $('#remamnt'+id).html(r_amnt1.replace(/\d(?=(\d{3})+\.)/g, '$&,'));
            $('#apatd'+id).html('0.00');
            $('#apainpt'+id).val('0.00');
