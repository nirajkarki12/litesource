...
class Invoices extends Admin_Controller {
    ...
    function client_project_contact_status_index() {
        ...
        $contact = $this->common_model->query_as_object('SELECT contact_id id, contact_name n, contact_active a FROM mcb_contacts ORDER BY contact_name ASC');
        $project = $this->common_model->query_as_object('SELECT project_id id, project_name pn, project_specifier ps, project_active a FROM mcb_projects ORDER BY project_name ASC');
        $invoice_statuses = $this->common_model->query_as_object('SELECT * FROM mcb_invoice_statuses');
        $data = array(
            'clients' => $clients,
            'projects' => $project,
            'contacts' => $contact,
    ...
    function index() {
        ...
        $limit = 200;
        $quote_invoice_support = $this->client_project_contact_status_index();
        //---- check is quote or invoices ------
        $is_quote = (uri_seg_is('quotes') ? 1 : 0);
        if($is_quote == 1){
            $data['quote_invoice'] = json_encode(array_merge($quote_invoice_support, $this->quote_invoice_index_quote($limit)));
        }else{
            $data['quote_invoice'] = json_encode(array_merge($quote_invoice_support, $this->quote_invoice_index_invoice(100)));
        }
    ...
    function get_quote_invoices_JSON_index() {
        ...
        if($is_quote == 1){
            $data['quote_invoice'] = $this->quote_invoice_index_quote($limit);
        }else{
            $data['quote_invoice'] = $this->quote_invoice_index_invoice($limit);
        }
        echo json_encode($data['quote_invoice']);
    }
...
//        }
        ...
        $invoice_query = "SELECT SQL_CALC_FOUND_ROWS 
                            ...
                            ORDER BY {$sort_order_} {$sort_by_} LIMIT 20000";
