...
class Mdl_Delivery_Dockets extends MY_Model {
    ...
    public function delete($docket_id) {
        ...
        //update status for pending inventory
        //****pending inv*****
        // The delivery docket is created assuming full quantity are supplied. So inserting new rows for mcb_item_stock
        $docketItems = $this->getDocketItemsByDocketId($docket_id);
        if(sizeof($docketItems) > 0){
            foreach($docketItems as $item){
                $sql = "update mcb_item_stock set status='0',last_modified_date='".date('Y-m-d H:i:s')."', modified_by='".$this->session->userdata('user_id')."' where relevent_item_id='".$item->docket_item_id."' AND relevent_item_field='docket_item_id' AND qty_update_source='docket-delivery'";
                $this->db->query($sql);
            }
        }
    ...
    function docket_payment_history_by_invoice_id($invoice_id) {
        $sql = "SELECT mdd.docket_id, mdd.docket_number, mdd.paid_status, 
                mddp.amount_entered, mddp.amount_log, mddp.note, mddp.time, mddp.user_name 
                FROM `mcb_delivery_dockets` AS mdd 
                LEFT JOIN mcb_delivery_docket_payment AS mddp 
                ON mdd.docket_id = mddp.docket_id 
                WHERE `invoice_id` = '$invoice_id' AND `mddp`.`id` > '0' ORDER BY `mddp`.`time` DESC";
    ...
    public function updateDocketPrice($item){
        $invoice_id = $item['item']['l_data']->invoice_id;
        $price_with_tax = filter_var($item['invoice_amounts']->invoice_total, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
        
        /// getting all docket
//        $dcts = $this->get_Where('mcb_delivery_dockets', array('invoice_id'=>$invoice_id));
//        if( $dcts != NULL ){
//            
//            foreach ($dcts as $dct) {
//                
//                if( $dct['paid_status'] != '1' ){
//                    
//                }
//                
//                echo '<pre>';
...
//        }
        ...
        //$this->db->query("SELECT * `mcb_delivery_dockets`  WHERE `invoice_id` = '{$invoice_id}'");    
        
        $this->db->query("UPDATE `mcb_delivery_dockets` SET `price_with_tax` = '{$price_with_tax}' WHERE `invoice_id` = '{$invoice_id}' AND `paid_status` != '1'");    
    }
    /**
    ...
    public function update_docket_item($docket_id, $docket_item) {
        ...
        $sql = "update mcb_item_stock set qty_pending = '-".$docket_item->docket_item_qty."', last_modified_date='".date('Y-m-d H:i:s')."',modified_by='".$this->session->userdata('user_id')."' where relevent_item_id='".$docket_item->docket_item_id."' AND qty_update_source='docket-delivery' AND relevent_item_field='docket_item_id'";
        $this->db->query($sql);
        // ======================end==============
        return $docket_item;
    }
    ...
    public function create_invoice_docket($invoice_id, $docket_date_entered, $strtotime = TRUE) {
        ...
        $sql_udq = "SELECT ? docket_id, i.invoice_item_id, i.item_qty - IFNULL(delivered_item_qty,0) undelivered_item_qty, i.item_index, i.item_length 
        ...
        if ($max_udq > 0) {
            ...
            if(sizeof($docketItems) > 0){
                foreach($docketItems as $item){
                    $products_inventory1 = $this->mdl_delivery_dockets->get_Where('mcb_products_inventory',array('product_id'=>$item->inventory_id));
                    
                    foreach ($products_inventory1 as $prodct_inmentry) {
                        $quqntity = $prodct_inmentry['inventory_qty']*$item->docket_item_qty;
                        $sql = "insert into mcb_item_stock(inventory_id, qty_update_source, relevent_item_field,`relevent_item_id`, qty_pending, `last_modified_date`,status, modified_by) values "
                            . "('".$prodct_inmentry['inventory_id']."', 'docket-delivery', 'docket_item_id','".$item->docket_item_id."', '-".$quqntity."', '".date('Y-m-d H:i:s')."','0', '".$this->session->userdata('user_id')."')";
                        $res = $this->db->query($sql);
                        if(!$res){
                            echo 'Error executing query: ';print_r($sql);
                        }
                    }
    ...
    function getDocketItemsByDocketId($docket_id){
        $q = $this->db->query("select di.*,ii.product_id as inventory_id from mcb_delivery_docket_items as di left join mcb_invoice_items as ii on di.invoice_item_id = ii.invoice_item_id where di.docket_id='".$docket_id."'");
        return $q->result();
    }
    ...
    function create_docket_items($docket_id, $invoice_id, $sql_udq) {
        ...
        $sql = "INSERT
			   INTO mcb_delivery_docket_items (docket_id, invoice_item_id, docket_item_qty, docket_item_index, docket_item_length)
     		 $sql_udq";
    ...
    function get_docket_owing_amount_status($docket_id) {
        $sql = "SELECT ROUND(( mcb_delivery_dockets.price_with_tax),2) AS price_with_taxes, ROUND(( SUM(mcb_delivery_docket_payment.amount_entered)),2) AS entered_amount, 
                IF( ROUND(( SUM(mcb_delivery_docket_payment.amount_entered)),2) IS NOT NULL, ROUND((mcb_delivery_dockets.price_with_tax - SUM(mcb_delivery_docket_payment.amount_entered)),2), ROUND(( mcb_delivery_dockets.price_with_tax),2) ) AS owing_amount
                FROM mcb_delivery_dockets
                LEFT JOIN mcb_delivery_docket_payment ON mcb_delivery_dockets.docket_id = mcb_delivery_docket_payment.docket_id 
                WHERE mcb_delivery_dockets.docket_id = '{$docket_id}'";
    ...
    function mark_as_paid_on_history($docket_id) {
        
        $lastid = 0;
        $amount_status = $this->get_docket_owing_amount_status($docket_id);
        $amount = $amount_status->owing_amount;
    ...
    function mark_as_unpaid_on_history($docket_id) {
        
        $check = $this->mdl_delivery_dockets->get_Row('mcb_delivery_docket_payment',array('docket_id'=>$docket_id, 'mark_as_paid'=>'1' ));
        if( isset($check->mark_as_paid) ){
            if( $check->mark_as_paid == '1' ){
                $amount = $check->amount_entered;
                $amount = (-1)*($amount);
                $username = $this->mdl_delivery_dockets->get_Row('mcb_users',array('user_id'=>$this->session->userdata('user_id')))->username;
                $data = array(
                    'docket_id' => $docket_id,
                    'note' => 'Marked as unpaid.',
                    'amount_entered' => $amount,
                    'amount_log' => $amount,
                    'time' => time(),
                    'user_name'=> $username
                ...
                $this->db->update('mcb_delivery_docket_payment', array('mark_as_paid'=>'0'));
                $this->db->trans_complete();   
            }
        }
        return TRUE;
    ...
    public function get_Row($tbl_name, $id_edt,$orderby,$order='asc',$limit='') {
