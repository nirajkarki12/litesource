        ...
        if( $clients != NULL ){
            foreach ($clients as $key => $client) {
                if( isset($client->value) ){
                    $clients[$key]->value = trim(trim($client->value, "'"));
                }
                if( isset($client->client_name) ){
                    $clients[$key]->client_name = trim(trim($client->client_name, "'"));
                }
                if( isset($client->supplier_name) ){
                    $clients[$key]->supplier_name = trim(trim($client->supplier_name, "'"));
                }
            }
        }
    ...
    public function save() {
        ...
        if ($client_name_exists) {
            ...
            $data = array(
                ...
                'client_name' => $this->input->post('client_name'),
                'parent_client_id' => (int)$this->input->post('parent_client_id'),
