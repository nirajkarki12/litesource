...
class Mdl_Inventory_Import extends MY_Model {
    ...
    public function process_inventory_data_file($source_file, $filename) {
        ...
        if (($handle = fopen($source_file, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 0, ",")) !== FALSE) {
                $temp = array();
                foreach ($data as $d) {
                    $val = str_replace( array("\xB0","\0","\xBA", "\x", "\x86"), "", $d);
                    $temp[] = $val;
                }
                $data = $temp;
                $num = count($data);
                ...
                if ($row > 1) {
                    if ( $num >= 10 ) {
                        ...
                        $duplicate_inventory = $this->is_duplicate_inventory($data[2]);
    ...
    private function addNewInventoryItem($data, $supplier_id) {
        ...
        if ($this->db->insert('mcb_inventory_item', $dataI)) {
            ...
            $history = array(
                'inventory_id' => $inventory_id,
                'history_qty' => $sign . ( floatval($data[8])),
                'notes' => 'Import ',
                'user_id' => $this->session->userdata('user_id'),
                'created_at' => date('Y-m-d H:i:s'),
            );
