...
//        die;
    ...
    function get_suplier_inventory_index() {
        ...
        $inventory_qry1 = "SELECT SQL_CALC_FOUND_ROWS 
                            ...
                            i.`location`, 
                            i.`o_qty`, 
                            -- '<div class=\"timeline-wrapper\"><div class=\"timeline-item\"><div class=\"animated-background\"></div></div></div>' as p_q,
                            '0' AS p_q,
                            ...
                            LEFT JOIN mcb_orders AS ord ON (oit.order_id = ord.order_id) WHERE oit.inventory_id = i.inventory_id AND 
                            -- ord.order_status_id != '3' 
                            ord.order_status_id = '2' AND oit.stock_status!='1') as o_q,
        ...
        $inventory = $this->common_model->query_as_object($inventory_qry1);
        ...
        $invtr_produsts = array();
        if( $qry_rip_chk ){
            $res_rip_chk = $qry_rip_chk->result_array();
            if ($res_rip_chk != NULL) {
                foreach ($res_rip_chk as $ip) {
                    if( !isset($invtr_produsts[$ip['iid']]) ){
                        $invtr_produsts[$ip['iid']] = array();
                    }
                    if( ($ip['pid'] == $ip['iid']) || ($ip['type'] > '0') ){
                        $invtr_produsts[$ip['iid']][] = $ip['pid'];
                    }
                }
            }
        }
        ...
        foreach ($inventory as $key => $invent) {
            if( $invent->i_t == 'Product Group'){
                ...
                $invent->p_q = 0;
                if( isset($invtr_produsts[$invent->id])  && ($invtr_produsts[$invent->id] != NULL) ){
                    $invent->p_q = $this->mdl_inventory_item->get_pending_qty_by_inv_id($invent->id, $invtr_produsts[$invent->id]);
                }
            }
        }
...
//        
//        $client_arr = array();
//        $clients_db = $this->common_model->query_as_object("SELECT client_id, client_name FROM mcb_clients");
//        if( $clients_db != NULL ){
//            foreach ($clients_db as $clint) {
//                $client_arr[$clint->client_id] = $clint->client_name;
//            }
//        }
//        
//        $inv_pros = array();
//        $qry_rip_chk = $this->common_model->query_as_array("SELECT 
//                mpi.product_id AS pid, 
//                mpi.inventory_id AS iid, 
//                mcb_inventory_item.inventory_type AS type 
//                FROM `mcb_products_inventory` AS mpi LEFT JOIN mcb_inventory_item 
//                ON mpi.product_id = mcb_inventory_item.inventory_id 
//                ");
//        if ($qry_rip_chk != NULL) {
//            foreach ($qry_rip_chk as $pi) {
//                $pid = $pi['pid'];
//                $iid = $pi['iid'];
//                if (!isset($inv_pros[$iid])) {
//                    $inv_pros[$iid] = array();
//                }
//                if (!in_array($pid, $inv_pros[$iid])) {
//                    if( ($pi['pid'] == $pi['iid']) || ($pi['type'] > '0') ){
//                        $inv_pros[$iid][] = $pid;
//                    }
//                }
//            }
...
//
////        $pro_invs = array();
////        $pro_invs_qry = $this->common_model->query_as_array("SELECT product_id AS pid, inventory_id AS iid FROM mcb_products_inventory WHERE product_id=inventory_id");
////        if( $pro_invs_qry != NULL ){
////            foreach ($pro_invs_qry as $p_inv) {
////                $pro_invs[$p_inv['pid']] = $p_inv;
////            }
////        }
//        
////        $sqlCnt = "select SUM(IF(mis.status='1', mis.qty_pending, 0)) as pending_qty, mpi.product_id 
////                from mcb_item_stock as mis 
////                inner join mcb_products_inventory as mpi on mpi.inventory_id = mis.inventory_id 
////                left join mcb_delivery_docket_items as ddi on ddi.docket_item_id = mis.relevent_item_id 
////                left join mcb_invoice_items as ii on ii.invoice_item_id = mis.relevent_item_id 
////                where ii.invoice_id >'0' GROUP BY mpi.product_id";
////        
...
//                            i.`use_length`, 
//                            i.`description`, 
//                            i.`supplier_price` AS s_p, 
//                            FORMAT(i.supplier_price, 2) AS s_p,
//                            i.`supplier_code` AS s_c,
//                            CONCAT('$', FORMAT(i.`base_price`, 2)) base_price, 
//                            i.`supplier_description`, 
//                            i.`location`, 
...
//                            -- ord.order_status_id != '3' 
//                            ord.order_status_id = '2' AND oit.stock_status!='1') as o_q,
//                            (IF(i.`inventory_type` = '1', (SELECT MIN(`mcb_inventory_item`.`qty`) 
...
//                            WHERE `mcb_products_inventory`.`product_id` = i.inventory_id), IF(i.qty IS NOT NULL, i.qty, '0.00'))) as qty
//                            FROM `mcb_inventory_item` AS i 
//                            WHERE i.is_arichved != '1'";
//        
//        
...
//                
//                /// set part type
//                $inventory[$key]->i_t = 'Part';
//                if( $invent->inventory_type == '1' ){
//                    $inventory[$key]->i_t = 'Product Group';
//                }
//                unset($inventory[$key]->inventory_type);
//                
//                /// set clent name//
//                $inventory[$key]->client_name = $client_arr[$invent->supplier_id];
//                
//                ///// suplier price
//                if( $invent->s_p == NULL){
//                    $inventory[$key]->s_p = '$0.00';
//                }else{
//                    $inventory[$key]->s_p = '$'.$invent->s_p;
//                }
//                
//                //// pending qty
//                $inventory[$key]->p_q = '0';
//                if( isset($inv_pros[$invent->id]) ){
//                    $inventory[$key]->p_q=$this->mdl_inventory_item->get_pending_qty($invent->id, $inv_pros[$invent->id]);
//                }
//                
////                if( $invent->inventory_type == 'Part' ){
////                    $inventory[$key]->p_q=$this->mdl_inventory_item->get_pending_qty($invent->id, $inv_pros[$invent->id]);
////                }else{
////                    
////                    $inventory[$key]->p_q = '0';
...
//                
//                
////                $inventory[$key]->pending_qty = '0';
////                if( isset($mcb_item_stock_arr[$invent->id]) ){
////                    $inventory[$key]->pending_qty = $mcb_item_stock_arr[$invent->id]->pending_qty;
////                }
//                
//            }
//            function sortByOrder($a, $b) {
...
//            $inventory = array_slice($inventory, 0,10000,true);
//        }
//        $this->load->model('clients/mdl_clients');
//        $data = array(
//            'suppliers' => $this->mdl_clients->get_active_suppliers(),
        ...
        if ($this->mdl_inventory_item->validate() && $this->mdl_inventory_history->validate()) {
         ...
         /*   if(!is_numeric($category) && !empty($category)){
            ...
            if (($this->input->post('use_length') == '1') && ($this->input->post('inventory_id') > '0')) {
                ...
                if ($inv_pro != NULL) {
                    $this->mdl_inventory_item->update('mcb_products_inventory_raw',array('category_id' => $category_id), array('inventory_qty' => '1'), array('inventory_id' => $this->input->post('inventory_id')));
                }
            }
...
//            $id = $this->mdl_inventory_item->form_value($inventory_id);
            ...
            if( $id > '0' ){
                $inventory_as_row = $this->common_model->query_as_object("
                    SELECT *,
                    st.stock_id,
                    SUM(IF(st.status='1',st.qty_pending,'0')) as qty_pending, 
                    (IF(i.inventory_type = '1', 
                    (SELECT MIN(`mcb_inventory_item`.`qty`) FROM `mcb_inventory_item` LEFT JOIN `mcb_products_inventory` ON `mcb_inventory_item`.`inventory_id` = `mcb_products_inventory`.`inventory_id` WHERE `mcb_products_inventory`.`product_id` = i.inventory_id), i.qty)) as quantity 
                    FROM mcb_inventory_item AS i  LEFT JOIN mcb_item_stock as st  on i.inventory_id = st.inventory_id  
                    WHERE i.inventory_id='" . $id . "' 
                    GROUP BY st.inventory_id  ORDER BY st.stock_id")[0] ;
                ...
                /// getting pending qty
                $inv_pros = array();
                $qry_rip_chk = $this->common_model->query_as_array("SELECT mpi.product_id AS pid, mpi.inventory_id AS iid, mcb_inventory_item.inventory_type AS type FROM `mcb_products_inventory` AS mpi LEFT JOIN mcb_inventory_item ON mpi.product_id = mcb_inventory_item.inventory_id WHERE mpi.`inventory_id` = '{$id}'");
                if ($qry_rip_chk != NULL) {
                ...
                $inventory_as_row->qty_pending = $this->mdl_inventory_item->get_pending_qty( $id,  $inv_pros);
