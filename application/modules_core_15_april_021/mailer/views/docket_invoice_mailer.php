        ...
        <div class="content toggle">
            ...
            <form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>">
                ...
                <dl>
                    ...
                    <dd>
                        <select onchange="change_contact()" name="default_to_contact" id="chng_contact">
                            <option value="">---select---</option>
                            <?php foreach($contacts as $contct){ ?>
                            <option data-c_name="<?=$contct->contact_name?>" data-c_email="<?=$contct->email_address?>" <?php if($contct->is_default == '1'){echo 'selected';} ?> value="<?=$contct->email_address?>"><?=$contct->email_address?><?php if($contct->is_default == '1'){echo '<span style="font-weight: bold;"> *</span>';} ?></option>
                            <?php } ?>
                        </select>
                    </dd>
                </dl>
...
function change_contact(){
    var c_name = ($('#chng_contact').find(':selected').data('c_name'));
    var c_email = ($('#chng_contact').find(':selected').data('c_email'));
    if(c_name != undefined){
