    ...
    function form() {
        ...
        if (!$this->mdl_mailer->validate_payment_email()) {
            ...
            if (!$_POST) {
                ...
                $this->mdl_payments->set_form_value('email_to', $invoice->client_email_address);
                $this->mdl_payments->set_form_value('email_from_name', $invoice->from_first_name . ' ' . $invoice->from_last_name);
                $this->mdl_payments->set_form_value('email_from_email', $invoice->from_email_address);
                $this->mdl_payments->set_form_value('email_subject', $this->lang->line('payment_receipt'));
                $this->mdl_payments->set_form_value('email_body', '');
                $this->mdl_payments->set_form_value('template', uri_assoc('receipt_template', 4));
            ...
            $this->mdl_mailer->email_payment_receipt($invoice, $template, $from_email, $from_name, $to, $subject, $email_body, $receipt_as_body, $email_cc, $email_bcc);
