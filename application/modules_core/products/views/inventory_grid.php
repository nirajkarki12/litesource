...
<script type="text/javascript">
    ...
    var inventory_items = <?php echo $inventory_items; ?>;
    ...
    var columns = [
        ...
        {id: "qty", name: "Total Inventory Qty", field: "qty", sortField: "", width: 50, sortable: true},
        {id: "inventory_qty", name: "Inventory Qty For This Product", field: "inventory_qty", sortField: "", width: 50, sortable: true},
        ...
        {id: "delCol", field: 'del', name: 'Action', width: 70, formatter: buttonFormatter},
    ];
    ...
    function addInventoryList() {
        ...
        var inv_qty = $('#qty_inventory_item').val();
        ...
        for (var i = 0, l = inventory_items.length; i < l; i++) {
            updateInventoryDetails(inventory_items[i],inv_qty);
        }
        dataView.beginUpdate();
        dataView.setItems(inventory_items, 'id');
        dataView.endUpdate();
        ...
        var node = document.createElement("input");
        node.value = parseInt($('#qty_inventory_item').val());
        node.setAttribute("type", "hidden");
    ...
    $(document).ready(function () {
        ...
        $.post("<?php echo site_url('inventory/get_inventory_JSON'); ?>", {
            ...
            if(grid_data == 'session_expired'){
                            window.location.reload();
                        }
            dataView.beginUpdate();
            var inventory_name = [];
            all_inventory_items = grid_data.inventory;
            for (var i = 0; i < all_inventory_items.length; i++) {
                inventory_name[i] = all_inventory_items[i]['name'];
            }
            suppliers = grid_data.suppliers;
