    ...
    $(document).ready(function (e) {
        ...
        $("#inventory_form").on('submit', function (e) {
            ...
            $.ajax({
                ...
                dataType: "json",
                async: false,
                url: "<?php echo site_url('inventory/inventory_has_relations'); ?>",
                data: {
                    id: inventory_id,
                },
                success: function(data) {
            ...
            let old_inventory_type = <?php echo $this->mdl_inventory_item->form_value('inventory_type');?>;
            let current_inventory_type = $('#inventory_type option:selected').val();

            if(old_inventory_type != current_inventory_type && isQuote) {
                if(!confirm('This item has related products and is already undergone to invoicing. This may affect the quantity calculation (Pending and Open Order qty).\n\n Are you sure to proceed?')){
                    e.preventDefault();
                    return false;
                }
            }
