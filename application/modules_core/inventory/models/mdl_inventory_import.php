                                            ...
                                            if ($c_data['inventory_type'] == 'Grouped') {
                                                ...
                                                if ($ri_count > 0) {
                                                    for ($x = 1; $x <= $ri_count; $x++) {
                                                        if (($c_data['related_inv_' . $x] != '') || ($c_data['related_inv_' . $x] != 0) || ($c_data['related_inv_' . $x] != NULL)) {
                                                            ...
                                                            if (($c_data['related_inv_' . $x .'_qty'] != '') || ($c_data['related_inv_' . $x .'_qty'] != 0) || ($c_data['related_inv_' . $x .'_qty'] != NULL)) {
                                                                $c_sku_qty = $c_data['related_inv_' . $x .'_qty'];
                                                            }
                                                            ...
                                                            if (!isset($sku_arrs[$c_sku])) {
                                                                ...
                                                                $csv_rel_invs_qty_arr[$c_sku] = ($c_sku_qty ?: '1');
                                                            }
                                                        }
                                                    }
                                                }
                                                ...
                                                if (count((array) $csv_rel_invs_arr) > 0) {
                                                    foreach ($csv_rel_invs_arr as $value) {
                                                        ...
                                                        $this->common_model->insert('mcb_products_inventory_raw', array('product_id' => $pro_i, 'inventory_id' => $inv_id, 'inventory_qty' => $csv_rel_invs_qty_arr[$value]));
                                                    }
                                                }
                                            }
...
//        die;
    ...
    public function export_updated_inventories($lastchanged_Ymd = null, $sort_by, $sort_order) {
        ...
        $pis = array();
        $product_inventory = $this->common_model->query_as_object("SELECT mcb_products_inventory.product_id, mcb_inventory_item.sku, mcb_products_inventory.inventory_qty as qty FROM `mcb_products_inventory` INNER JOIN mcb_inventory_item ON mcb_products_inventory.inventory_id = mcb_inventory_item.inventory_id;");
        foreach ($product_inventory as $pi) {
            $product_id = $pi->product_id;
            unset($pi->product_id);
            $pis[$product_id][] = ['sku'=>$pi->sku, 'qty'=>$pi->qty];
        }
        ...
        if ($max_rel_inv_count > 0) {
            for ($index = 1; $index <= $max_rel_inv_count; $index++) {
                $rel_key = "related_inv_" . $index;
                $heading[] = $rel_key;
                $rel_key = "related_inv_" . $index ."_qty";
                $heading[] = $rel_key;
            }
        }
        ...
        foreach ($q->result() as $inventory) {
            ...
            if ($max_rel_inv_count > 0) {
                for ($index = 1; $index <= $max_rel_inv_count; $index++) {
                    ...
                    $is_key = ($index - 1);
                    $rel_key = "related_inv_" . $index;
                    $rel_key_qty = $rel_key .'_qty';
                    $inventory->$rel_key = '';
                    $inventory->$rel_key_qty = '';

                    if ($inventory->inventory_type == '1') {
                        if (array_key_exists($inv_id, $pis) && array_key_exists($is_key, $pis[$inv_id])) {
                        // if (isset($pis[$inv_id][$is_key])) {
                            $inventory->$rel_key = $pis[$inv_id][$is_key]['sku'];
                            $inventory->$rel_key_qty = $pis[$inv_id][$is_key]['qty'];
                        }
                    }
                }
            }
        ...
        foreach ($invs as $inventory) {
            ...
            if ($max_rel_inv_count > 0) {
                for ($index = 1; $index <= $max_rel_inv_count; $index++) {
                    ...
                    if ($inventory->inventory_type == '1') {
                        $line[] = prepare_csv_export_string($inventory->$rel_key);
                        $line[] = prepare_csv_export_string($inventory->$rel_key_qty);
                    }
                }
            }
            fputcsv($file, $line, $delimiter, $enclosure);
