...
class Invoice_Settings extends Admin_Controller {
	...
	function display() {
		...
		$data = array(
			...
			'addresses'					=>  $this->mdl_addresses->get_default_addresses(),
	...
	function save() {
		...
		$this->mdl_mcb_data->save('default_pdf_info_user_id', $this->input->post('default_pdf_info_user_id'));
		$this->mdl_mcb_data->save('default_order_address_id', $this->input->post('default_order_address_id'));
		...
		$this->mdl_mcb_data->save('tax_id_number_label', 'ABN');
		...
		if ($this->input->post('disable_invoice_payments')) {
			...
			$this->mdl_mcb_data->save('disable_invoice_payments', 0);
