    ...
    function check_order_creation(){
        ...
        if (sizeof($invoice_items) > 0) {
            foreach ($invoice_items as $i_item) {
                ...
                if ($i_item->supplier_id == '0' || $i_item->supplier_id == NULL) {
        ...
        if($redirect_to_product){
            echo json_encode(array('result' => 'problem_redirect','detail' => 'New products detected, Click OK to create invoice or Cancel to update new products.')); exit;
        }
    ...
    function quote_to_orders_invoice() {
        ...
        if (sizeof($invoice_items) > 0) {
            ...
            foreach ($invoice_items as $i_item) {
                ...
                //if there is new product then we are going to add it
                //if (($i_item->product_id == '0' || $i_item->product_id == NULL) && trim($i_item->item_name) != '') {
                    //checking duplication
                    $existing = $this->mdl_products->checkDuplicate_product_supplier_code_with_return($i_item->item_name);
                    if(!$existing){
                        $product_array = array(
                            'product_name' => $i_item->item_name,
                            'product_description' => $i_item->item_description,
                            'product_supplier_description' => '',
                            'product_supplier_code' => '',
                            'product_supplier_price' => $i_item->item_price,
                            'product_base_price' => ($i_item->item_price + $i_item->item_price*$invoice->client_group_discount_percent/100),
                            'product_active' => '1',
                        );
                        $product_id = $this->mdl_products->addNewProductFromQuote($product_array);
                        $new_product_count++;
...
//                    if($i_item->invoice_item_id == '272519'){
...
//                                }
                ...
                $this->session->set_flashdata('custom_success', 'Quote copied successfully.<br/>Inventory sufficient. Order was not created.');
                redirect('invoices/index');
            }
        }
...
//                var_dump($dockets);die();
        ...
        if (sizeof($dockets) > 0) {
            ...
            foreach ($dockets as $inv) {
                ...
                <form action="' . site_url() . '/delivery_dockets/generatedocketinvoice/docket_id/' . $inv->docket_id . '" style="float: left;margin-right: 10px;">
                    <button class="button btnopennewtab">
                        Generate Invoice
                    </button>
                </form>
                <form action="' . site_url() . '/delivery_dockets/senddocketinvoice/docket_id/' . $inv->docket_id . '">
                    <button class="button">
                        Send invoice
                    </button>
                </form>';
                ...
                if (($inv->docket_delivery_status) == 0 || $inv->docket_delivery_status == NULL) {
                    ...
                    $popHtml .= '<form action="' . site_url() . '/delivery_dockets/finalizedelivery/docket_id/' . $inv->docket_id . '">
                    <button class="button">
                        Finalize deleviery
                    </button>
                </form>';
                } else {
                    $popHtml .= '<form action="' . site_url() . '/delivery_dockets/canceldelivery/docket_id/' . $inv->docket_id . '">
                    <button class="button">
                        cancel deleviery
                    </button>';
                }
