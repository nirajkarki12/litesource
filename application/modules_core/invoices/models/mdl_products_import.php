...
//                }
    ...
    private function updateProductDetail($product_id, $data, $supplier_id) {
        ...
        if($this->db->update('mcb_products', $data)){
            $this->duplicateAsInventoryForUpdateProduct($product_id, $data);
            return TRUE;
        }
    }
    ...
    private function addNewProduct($data, $supplier_id) {
        ...
        if ($this->db->insert('mcb_products', $data)) {
            ...
            $this->duplicateAsInventoryForNewProduct($product_id, $data);
            return $product_id;
        }
        return FALSE;
    }
    ...
    function duplicateAsInventoryForNewProduct($product_id, $product_data) {
    ...
    function duplicateAsInventoryForUpdateProduct($product_id, $product_data) {
        ...
        $pro_inv_rel = $this->common_model->get_row('mcb_products_inventory', array('product_id'=> $product_id));
        if($pro_inv_rel == FALSE){
            $invDetail = $this->common_model->get_row('mcb_inventory_item', array('name'=> $data['name']));
            if($invDetail != FALSE ){
                $this->update('mcb_inventory_item', $data, array('inventory_id'=>($invDetail->inventory_id)));
                $c = array('product_id'=>$product_id, 'inventory_id'=>$invDetail->inventory_id);
                ...
                $inventory_id = $this->common_model->insert('mcb_inventory_item', $data);
            }
            $this->common_model->insert('mcb_products_inventory', array('product_id'=>$product_id, 'inventory_id'=>$inventory_id, 'inventory_qty'=>'1') );
        }
    }
