...
public function adjust_invoice_number_old($invoice_id, $invoice_group_id) {
	...
	public function adjust_invoice_number($invoice_id) {
		
		$invoice = $this->mdl_invoices->get_by_id($invoice_id);
		$invoice_group_id = $invoice->invoice_group_id; //1;
		
		$group = parent::get_by_id($invoice_group_id);

	
		$invoice_number = $group->invoice_group_next_id;

		log_message('INFO', 'Got invoice_number: '.$invoice_number);
