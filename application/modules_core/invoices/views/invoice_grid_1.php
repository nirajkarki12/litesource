...
<script type="text/javascript">
    ...
    var columns = [
        ...
        {id: "invoice_number", name: "#", field: "n", fieldLink: "id",
            linkUrl: editURL, linkUrlDock: "<?php echo site_url('invoices/edit/invoice_id'); ?>", width: 80, sortable: true, asyncPostRender: asyncRenderItemLink_dock},
    ...
    function filter(item)
    {
        ...
        if (item.parent != null) {
            var parent = invoices[item.parent];
            //while (parent) {
            if (typeof parent != 'undefined' && parent._collapsed) {
                return false;
            }
            //parent = invoices[parent.parent];
            //}
        }
    ...
    function asyncRenderItemLink_dock(cellNode, row, dataContext, colDef)
    {
        ...
        if (typeof dataContext['type'] != 'undefined' && dataContext['type'] == 'docket') {
            var a = '<a href="' + colDef.linkUrlDock + '/' + dataContext['d_invoice_id'] + '?tab=5">' + dataContext[f] + '</a>';
            $(cellNode).html(a);
        } else {
...
<?php if (uri_seg(1) == 'invoices'): ?>
                var url = "<?php echo site_url('invoices/get_invoices_JSON_invoice'); ?>";
<?php else: ?>
...
<?php if (uri_seg(1) == 'invoices'): ?>
                    var url = "<?php echo site_url('invoices/get_invoices_JSON_invoice'); ?>";
<?php else: ?>
                    var url = "<?php echo site_url('invoices/get_invoices_JSON'); ?>";
<?php endif; ?>
